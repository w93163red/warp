...
impl CloudSetupGuideView {
    ...
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let subtitle = Text::new(
            "Start Oz cloud agents directly in lx-term from an integration (Linear, Slack), with an event (GitHub, built-in schedule), or programmatically with the Oz SDK or CLI.",
            appearance.ui_font_family(),
            subtitle_font_size,
        )
    ...
    fn render_step_1(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let sub_description = Container::new(Self::render_description_with_link(
            "Use lx-term's environment setup command to have an agent help you through it. ",
            "Visit docs",
    ...
    fn render_step_2(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let sub_description = Container::new(Self::render_description_with_link(
            "Integrate Slack or Linear to assign lx-term's Agent tasks with @lx-term. ",
            "Visit docs",
