...
fn enabled_features() -> HashSet<FeatureFlag> {
    ...
    // This fork ships without telemetry, crash reporting, autoupdate, or a
    // login requirement, so the flags that gate them stay off no matter what
    // the channel or the compiled-in features enable.
