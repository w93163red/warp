...
/// Unused while this fork's [`ServerApi::fetch_channel_versions`] is stubbed out:
/// network version checks are disabled so nothing pings Warp's servers. Kept so
/// restoring that request does not have to re-pick a timeout.
#[allow(dead_code)]
...
pub enum AIApiError {
    ...
    #[error("lx-term is currently overloaded. Please try again later.")]
    ServerOverloaded,
