...
impl AgentSlide {
    ...
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let subtitle = FormattedTextElement::from_str(
            "Select your lx-term Agent's defaults.",
    ...
    fn render_autonomy_section(
        ...
        let autonomy_options: [(AgentAutonomy, &str, &str, MouseStateHandle); 3] = [
            (
                AgentAutonomy::Full,
                "Full",
                "lx-term Agent runs commands, writes code, and reads files without asking.",
                self.autonomy_full_mouse_state.clone(),
            ),
            (
                AgentAutonomy::Partial,
                "Partial",
                "lx-term Agent can plan, read files, and execute low-risk commands. Asks before making any changes or executing sensitive commands.",
                self.autonomy_partial_mouse_state.clone(),
            ),
            (
                AgentAutonomy::None,
                "None",
                "lx-term Agent takes no actions without your approval.",
