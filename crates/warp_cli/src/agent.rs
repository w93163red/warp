...
/// Excludes [`Harness::Oz`]: this fork ships without the Warp-hosted harness, so
/// it stays deserializable for old/server data but is not offered on the CLI.
const HARNESS_VALUE_VARIANTS: [Harness; 4] = [
    Harness::Claude,
    Harness::OpenCode,
    Harness::Gemini,
    Harness::Codex,
...
impl Harness {
    ...
    pub fn should_display_in_help_text(self) -> bool {
        match self {
            ...
            Self::Oz | Self::OpenCode | Self::Gemini | Self::Unknown => false,
        }
    }
...
pub struct RunAgentArgs {
    ...
    ///
    /// To automate a skill on a schedule, use `lx-term schedule create --skill <SKILL>`.
...
pub struct RunCloudArgs {
    ...
    /// Execution harness for the agent run.
    ///
    /// "codex" (the default) delegates the run to the `codex` CLI, as do the
    /// other values; see the possible values below.
    #[arg(long = "harness", value_name = "HARNESS", default_value_t = Harness::Codex)]
    pub harness: Harness,
